...
public class MamagerTrap : MonoBehaviour {
	...
	public void createLeftTrap()
	{
		...
		{ 
			...
			if(last1.transform.position.z-Camera.main.transform.position.z <60)
					last1 = creaNewTrap(true);
			if(last2.transform.position.z-Camera.main.transform.position.z <60)
					last2 = creaNewTrap(false);
		}
...
//		Debug.Log (i);
		...
		if (left) {
			if(last2 == null)
				...
				offsetz= PlatformManager.instance.posLand1.position.z + 5;
			else 
...
//				trap.anim.Play ("Trap_Left_0_IDE");
	...
	//			trap.anim.Play ("Trap_Left_1_IDE");
			...
			if(last2 == null)
				...
				offsetz= PlatformManager.instance.posLand4.position.z + 5;
			else 
	...
	//	trap.type = type;
		...
		obj.transform.Translate (offsetx, 0, f + offsetz + 5,Space.World);
