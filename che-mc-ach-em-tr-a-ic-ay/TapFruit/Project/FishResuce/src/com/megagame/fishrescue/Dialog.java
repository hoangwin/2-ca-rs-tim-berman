...
import com.megagame.fishrescue.state.StateMainMenu;
...
public class Dialog
{
	...
	//
	public static int DIALOG_CONFIRM_X = FishTap.SCREEN_WIDTH / 20;
	public static int DIALOG_CONFIRM_Y = FishTap.SCREEN_HEIGHT / 4;
	public static int DIALOG_CONFIRM_W = FishTap.SCREEN_WIDTH - 2 * FishTap.SCREEN_WIDTH /20;
	public static int DIALOG_CONFIRM_H = FishTap.SCREEN_HEIGHT - 2 * FishTap.SCREEN_HEIGHT / 4;
	...
	{
		int offset =0;
		if(FishTap.SCREEN_WIDTH<600)
		{
			offset = DIALOG_CONFIRM_H/8;
		}
		FishTap.mainPaint.setStyle(Style.FILL);
		FishTap.mainPaint.setARGB(200, 0, 0, 0);
		c.drawRect(0, 0, FishTap.SCREEN_WIDTH, FishTap.SCREEN_HEIGHT, FishTap.mainPaint);
		FishTap.mainPaint.setStyle(Style.FILL_AND_STROKE);
		FishTap.mainPaint.setColor(Color.argb(170,0, 0, 0));
		c.drawRoundRect(rectF, 25, 25, FishTap.mainPaint);
		//draw text
		FishTap.fontbig_White.drawString(c, dialogText, FishTap.SCREEN_WIDTH / 2, DIALOG_CONFIRM_Y+ offset , DIALOG_CONFIRM_W - 20, BitmapFont.ALIGN_CENTER);
		...
		case DIALOG_TYPE_CONFIRM:
			...
			if (FishTap.isTouchDrapInRect(x1, y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(FishTap.mainCanvas, DEF.FRAME_CANCEL_HIGHTLIGHT, x1, y);
			else
				StateGameplay.spriteDPad.drawAFrame(FishTap.mainCanvas, DEF.FRAME_CANCEL_NORMAL, x1, y);

			if (FishTap.isTouchDrapInRect(x2, y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))
			   
				StateGameplay.spriteDPad.drawAFrame(FishTap.mainCanvas, DEF.FRAME_OK_HIGHTLIGHT, x2, y);
			else
				StateGameplay.spriteDPad.drawAFrame(FishTap.mainCanvas, DEF.FRAME_OK_NORMAL, x2, y);
			break;
	...
	public static void updateDialog()
	{
		...
		case DIALOG_TYPE_CONFIRM:
			...
			if (FishTap.isTouchReleaseInRect(x1 , y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))//cancel
			{
				hideDialog();
			} else if (FishTap.isTouchReleaseInRect(x2 , y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))//ok
			{
				Dialog.hideDialog();
				...
				case DIALOG_NEXTSTATE_EXIT:
					...
					FishTap.mainActivity.finish();
					break;
				case DIALOG_BACK_TO_MAINMENU_EXIT:
					FishTap.changeState(FishTap.STATE_MAINMENU, true, true);
					break;
				case DIALOG_RESTART_GAME:
					FishTap.changeState(FishTap.STATE_GAMEPLAY, true, true);
				break;
				default:
					break;
				}
			...
			break;
		case DIALOG_TYPE_NOTICE:
			if (FishTap.isTouchReleaseInRect(x2 , y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))//ok
			{
				switch (dialogNextState)
				{
				case DIALOG_NEXTSTATE_CLOSE_LEADERBOARD_DIALOG:
					hideDialog();
					FishTap.changeState(FishTap.mPreState);
					break;
				//case DIALOG_BACK_TO_MAINMENU_EXIT:
				//	break;
