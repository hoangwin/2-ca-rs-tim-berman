...
public class StateGameplay extends ChemFish
{
	...
	public static synchronized void SendMessage(int type)
	{
		...
		case MESSAGE_UPDATE:
			...
			if(isNextlevel) 
			{
				if(ChemFish.isTouchPressScreen())
					isNextlevel = false;
				return;
			}
			...
			if (isKeyReleased(KeyEvent.KEYCODE_9)) {
				...
				ChemFish.changeState(IConstant.STATE_WINLOSE);
	...
	public static void drawHUD(Canvas c)
	{
		...
		{		
			if (isTouchDrapInRect(DEF.BUTTON_IGM_X, DEF.BUTTON_IGM_Y, DEF.BUTTON_IGM_W, DEF.BUTTON_IGM_H))
				spriteDPad.drawAFrame(ChemFish.mainCanvas, DEF.FRAME_PAUSE_HIGHTLIGHT, DEF.BUTTON_IGM_X, DEF.BUTTON_IGM_Y);
			else
				spriteDPad.drawAFrame(ChemFish.mainCanvas, DEF.FRAME_PAUSE_NORMAL, DEF.BUTTON_IGM_X, DEF.BUTTON_IGM_Y);
		}
