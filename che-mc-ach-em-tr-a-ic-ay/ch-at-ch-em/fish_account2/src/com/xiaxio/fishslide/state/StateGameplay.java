...
public class StateGameplay extends FishSlide
{
	...
	public static synchronized void SendMessage(int type)
	{
		...
		case MESSAGE_UPDATE:
			...
			if(isNextlevel) 
			{
				if(FishSlide.isTouchPressScreen())
					isNextlevel = false;
				return;
			}
			...
			if (isKeyReleased(KeyEvent.KEYCODE_9)) {
				...
				FishSlide.changeState(IConstant.STATE_WINLOSE);
	...
	public static void drawHUD(Canvas c)
	{
		...
		{		
			if (isTouchDrapInRect(DEF.BUTTON_IGM_X, DEF.BUTTON_IGM_Y, DEF.BUTTON_IGM_W, DEF.BUTTON_IGM_H))
				spriteDPad.drawAFrame(FishSlide.mainCanvas, DEF.FRAME_PAUSE_HIGHTLIGHT, DEF.BUTTON_IGM_X, DEF.BUTTON_IGM_Y);
			else
				spriteDPad.drawAFrame(FishSlide.mainCanvas, DEF.FRAME_PAUSE_NORMAL, DEF.BUTTON_IGM_X, DEF.BUTTON_IGM_Y);
		}
