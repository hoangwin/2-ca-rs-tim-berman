	...
	public void run() {		
			while (FishSlide.running) {
				...
				if ((timeCurrent - timePrev) > 1000 / FRAME_RATE_TARGET) {
					...
					FishSlide.SendMessage(FishSlide.mCurrentState,FishSlide.MESSAGE_PAINT);
					if(GameLib.mainCanvas != null && holder != null)
					{
						holder.unlockCanvasAndPost(GameLib.mainCanvas);
						FishSlide.SendMessage(FishSlide.mCurrentState,FishSlide.MESSAGE_UPDATE);
						FishSlide.UpdateKey();					
						FishSlide.UpdateTouch();
						timePrev2 = timePrev;
						timePrev = timeCurrent;
					}
