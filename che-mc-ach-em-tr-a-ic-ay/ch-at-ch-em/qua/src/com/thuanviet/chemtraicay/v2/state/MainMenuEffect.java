...
public class MainMenuEffect
{
	...
	{
		MINI_MUN_SPEED = 12;
		if (ChemFruit.SCREEN_HEIGHT < 400)
			MINI_MUN_SPEED = 4;
		else if (ChemFruit.SCREEN_HEIGHT < 600)
			MINI_MUN_SPEED = 6;
		else if (ChemFruit.SCREEN_HEIGHT < 700)
			MINI_MUN_SPEED = 8;
		else if (ChemFruit.SCREEN_HEIGHT < 1024)
			MINI_MUN_SPEED = 10;
		else if (ChemFruit.SCREEN_HEIGHT < 1280)
			MINI_MUN_SPEED = 11;
	...
	public static void Update()
	{
		...
		countTimeRandomFruitObject = (long) (1000 - ChemFruit.mcurrentlevel * 40);

		if (GameViewThread.timeCurrent - lastTimeCreateSpecial > countTimeCreateSpecial) {
			int _type = 12 + random.nextInt(3);
			int _x = random.nextInt(ChemFruit.SCREEN_WIDTH - (int)(ChemFruit.scaleX* 100));
