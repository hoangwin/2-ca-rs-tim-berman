...
import com.thuanviet.chemtraicay.v2.actor.Actor;
...
public class Sprite
{
	...
	public void drawAModule(Canvas g, int id, int angle, float Scalex, float Scaley, int flagX,int flagY, int x, int y)//flab la flib
	{

		matrix.reset();
		// rotation at center of module image
		matrix.postRotate(angle, modules[id].width / 2, modules[id].height / 2);
		matrix.postScale(Scalex* flagX, Scaley *flagY);
		if(flagX ==-1)
			matrix.postTranslate(x+modules[id].width, y); // move it into x, y position
		else
	...
	public void drawAModule(Canvas g, int id, int x, int y)//flab la flib
	{	
		g.drawBitmap(modules[id].image, x,y, paint);
		// g.drawBitmap(modules[id].image, x, y, null);
	}
	

	public void drawAFrame(Canvas g, int id, int angle, float Scalex, float Scaley, int flibX,int flibY ,int x, int y)
	{
		for (int i = 0; i < frames[id].numofModule; i++) {
			...
			if (angle != 0) {
				...
				drawAModule(g, moduleisPaint, angle, Scalex, Scaley,flibX,flibY, x + new_x, y + new_y);
			} else
				drawAModule(g, moduleisPaint, angle, Scalex, Scaley,flibX,flibY, (int) (x + frames[id].postionX[i] * Scalex), (int) (y + frames[id].postionY[i] * Scaley));
		}
	}
	...
	public void drawAnim(Canvas g, int angle, int Scalex, int Scaley,int flibX, int flibY, int x, int y)
	{
		if (currentFrame < anims[currentAnimation].numOfFrame)
			// drawAFrame(mainCanvas,0,20,1,1,200,200);
			drawAFrame(g, anims[currentAnimation].arrayFrameIndex[currentFrame], angle, Scalex, Scaley,flibX,flibY, x + anims[currentAnimation].postionX[currentFrame], y + anims[currentAnimation].postionY[currentFrame]);
		update();
	}
	...
	public void drawAnim(Canvas g, Actor actor, int angle, int Scalex, int Scaley,int flibX,int flibY, int x, int y)
	{

		if (actor._currentAnimation >= 0) {
			if (actor._currentFrame < anims[actor._currentAnimation].numOfFrame)
			// drawAFrame(mainCanvas,0,20,1,1,200,200);
			drawAFrame(g, anims[actor._currentAnimation].arrayFrameIndex[actor._currentFrame], angle, Scalex, Scaley,flibX,flibY, x + anims[actor._currentAnimation].postionX[actor._currentFrame], y + anims[actor._currentAnimation].postionY[actor._currentFrame]);
			else if (actor._ShowLastFrame) {
				int frameIndex = anims[actor._currentAnimation].numOfFrame - 1;
			
				drawAFrame(g, anims[actor._currentAnimation].arrayFrameIndex[frameIndex], angle, Scalex, Scaley,flibX,flibY, x + anims[actor._currentAnimation].postionX[frameIndex], y + anims[actor._currentAnimation].postionY[frameIndex]);
	...
	public void drawAnim(Canvas g, Actor actor)
	{
		if (actor._currentAnimation >= 0) {
			if (actor._currentFrame < anims[actor._currentAnimation].numOfFrame)
				drawAFrame(g, anims[actor._currentAnimation].arrayFrameIndex[actor._currentFrame], actor._xposSprite + anims[actor._currentAnimation].postionX[actor._currentFrame], actor._yposSprite + anims[actor._currentAnimation].postionY[actor._currentFrame]);
			update(actor);
		}
	}
