	...
	public void run() {		
			while (FruitSlide.running) {
				...
				if ((timeCurrent - timePrev) > 1000 / FRAME_RATE_TARGET) {
					...
					FruitSlide.SendMessage(FruitSlide.mCurrentState,FruitSlide.MESSAGE_PAINT);
					if(GameLib.mainCanvas != null && holder != null)
					{
						holder.unlockCanvasAndPost(GameLib.mainCanvas);
						FruitSlide.SendMessage(FruitSlide.mCurrentState,FruitSlide.MESSAGE_UPDATE);
						FruitSlide.UpdateKey();					
						FruitSlide.UpdateTouch();
						timePrev2 = timePrev;
						timePrev = timeCurrent;
					}
